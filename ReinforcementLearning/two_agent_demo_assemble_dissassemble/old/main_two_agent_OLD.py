...
class RobotBigGripper(object):
    ...
    def open_gripper(self, mm=850):
        try:
            code = self._arm.set_gripper_position(mm, wait=True, speed=2500, auto_enable=True)
    ...
    def test(self, _target_position, yaw_rotation):
        try:
            self._tcp_speed = 71
            self._tcp_acc = 1000
            self._angle_speed = 30
    ...
    def two_agent_first_task(self, _target_position, _destination_position, yaw_rotation):
        try:
            ...
            # move block to conveyor belt (black platform)
            ...
            # go down
            code = self._arm.set_position(*[_destination_position[0] - 15, _destination_position[1]-17, _destination_position[2] - 50, 180.0, 0.0, -91.5], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=-1.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            
            self.open_gripper(350)
            
            # go up
            code = self._arm.set_position(*[_destination_position[0], _destination_position[1], _destination_position[2] + 40, 180.0, 0.0, -91.5], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=-1.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            ...
            # go down
            code = self._arm.set_position(*[_destination_position[0], _destination_position[1], _destination_position[2] - 35, 180.0, 0.0, -91.5], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=-1.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
...
if __name__ == '__main__':
    ...
    first_agent.test(_target_position, yaw_rotation)
    exit()
