...
x_rot = 180
R_X = [[1,0,0], [0, np.cos(get_radian(x_rot)), -np.sin(get_radian(x_rot))], [0, np.sin(get_radian(x_rot)), np.cos(get_radian(x_rot))]]

# around z axis to get
z_rot = 1.6  # 1.6
R_Z = [[np.cos(get_radian(z_rot)), -np.sin(get_radian(z_rot)), 0], [np.sin(get_radian(z_rot)), np.cos(get_radian(z_rot)) , 0], [0,0,1]]
...
image_x = [188.5, 172.5, 534, 35, 195, 164, 211, 180, 163, 163, 211]
image_y = [210.5, 145, 336, 365, 158, 187, 216, 259, 168, 260, 180]
mm_x = 0.545 # x was better at 0.54
mm_y = 0.538 # y is better now than 0.54
image_x_mm = image_x[10] / mm_x
image_y_mm = image_y[10] / mm_y
...
"""
results:  (where mm_x = 0.545, mm_y=0.538 , x_rot=180, y_rot=1.6, trans vect = [-342, 702]) 
